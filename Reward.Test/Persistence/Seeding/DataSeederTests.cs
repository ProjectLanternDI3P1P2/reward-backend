...
public sealed class DataSeederTests
{
    ...
    public async Task SeedAsync_EmptyDatabase_CreatesBrowsableInventoryDataOnlyOnce()
    {
        ...
        int itemInstancesAfterFirstSeed = await context.ItemInstances.CountAsync(TestContext.Current.CancellationToken);
        await DataSeeder.SeedAsync(context, TestContext.Current.CancellationToken);

        // Assert
        (await context.Inventories.CountAsync(TestContext.Current.CancellationToken)).Should().Be(3);
        ...
        (await context.Equipment.CountAsync(TestContext.Current.CancellationToken)).Should().BeGreaterThan(0);
        (await context.ItemInstances.CountAsync(TestContext.Current.CancellationToken)).Should().Be(itemInstancesAfterFirstSeed);
    }
