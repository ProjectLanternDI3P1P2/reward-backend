...
public sealed class ItemClassTagConfiguration : IEntityTypeConfiguration<ItemClassTag>
{
    public void Configure(EntityTypeBuilder<ItemClassTag> builder)
    {
        ...
        builder.HasOne(x => x.Item).WithMany(x => x.ClassTags).HasForeignKey(x => x.ItemId).OnDelete(DeleteBehavior.Cascade);
        builder.HasOne(x => x.ClassTag).WithMany(x => x.Items).HasForeignKey(x => x.ClassTagId).OnDelete(DeleteBehavior.Cascade);
    }
}
