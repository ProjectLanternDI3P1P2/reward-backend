...
{
    private static readonly string[] CategoryLabels = ["WEAPON", "ARMOR", "SHIELD", "RING", "AMULET", "POTION", "VIAL"];

    public static IReadOnlyList<Category> CreateCategories(DateTimeOffset now) =>
        CategoryLabels.Select(label => new Category
        {
            Id = Guid.NewGuid(),
            Label = label,
            CreatedAt = now,
    ...
    public static IReadOnlyList<Item> GenerateItems(
        ...
        new Faker<Item>("en")
            ...
            .RuleFor(item => item.CategoryId, faker => categories[faker.Random.Int(0, categories.Count - 1)].Id)
            .RuleFor(item => item.RarityId, faker => rarities[faker.Random.Int(0, rarities.Count - 1)].Id)
            .RuleFor(item => item.Name, faker => $"{faker.Commerce.Color()} {faker.Commerce.ProductMaterial()} {faker.Commerce.ProductName()}")
            ...
            .RuleFor(item => item.Stackable, (_, item) => categories.Single(category => category.Id == item.CategoryId).Label is "POTION" or "VIAL")
